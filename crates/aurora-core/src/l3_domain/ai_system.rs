...
mod tests {
    ...
    /// 校验 V19 §7.2「本地 AI 不可用降级云端」配套的 `Error::AiInference`
    /// 行为契约：满足自动重试（短熔断/抖动重试），并明确需要降级，
    /// 与 V19 §33.2 降级矩阵保持一致。
    #[test]
    fn test_ai_inference_error_is_retryable_and_requires_fallback() {
        let err = crate::Error::AiInference("test: ollama unreachable".into());
        assert!(
            err.is_retryable(),
            "AiInference 必须可重试 (V19 §33.1)，让路由层短熔断/抖动重试"
        );
        assert!(
            err.requires_fallback(),
            "AiInference 必须需要降级 (V19 §33.2)：本地 AI 不可用时跳到云端 Provider"
        );
        // user_message 不应误导成「网络同步」文案 — 这是 AI 场景。
        let msg = err.user_message();
        assert!(
            msg.contains("AI"),
            "user_message 应说明是 AI 推理失败，实际: {msg}"
        );
    }
}
