...
mod tests {
    ...
    #[tokio::test]
    async fn test_registry_unregister_agent_removes_tools() {
        let r = ToolRegistry::new();
        let agent = make_agent("a1");
        r.register_agent("a1", agent as Arc<dyn AgentProtocol>);
        r.register_tool(make_tool("t1", "a1", &[])).await.unwrap();
        r.register_tool(make_tool("t2", "a1", &[])).await.unwrap();
    ...
    #[tokio::test]
    async fn test_registry_register_tool_without_agent_id_fails() {
        let r = ToolRegistry::new();
        let tool = Tool::new("t", "d", serde_json::json!({}));
        let err = r.register_tool(tool).await.unwrap_err();
    ...
    #[tokio::test]
    async fn test_registry_invoke_routes_to_agent() {
        let r = ToolRegistry::new();
        let agent = make_agent("a1");
        agent.register_handler("echo", |args| Ok(args.clone()));
        r.register_agent("a1", agent as Arc<dyn AgentProtocol>);
        r.register_tool(make_tool("echo", "a1", &[])).await.unwrap();

        let inv = ToolInvocation::new("echo", serde_json::json!({"msg": "hi"}), "s1");
        let result = r.invoke(&inv).await.unwrap();
        assert!(result.is_ok());
    ...
    #[tokio::test]
    async fn test_registry_invoke_unknown_tool() {
        let r = ToolRegistry::new();
        let inv = ToolInvocation::new("ghost", serde_json::json!({}), "s1");
        let err = r.invoke(&inv).await.unwrap_err();
        assert!(matches!(err, crate::Error::NotFound(_)));
    }

    #[tokio::test]
    async fn test_registry_invoke_handler_error_propagated() {
        let r = ToolRegistry::new();
        let agent = make_agent("a1");
        agent.register_handler("fail", |_| Err("boom".to_string()));
        r.register_agent("a1", agent as Arc<dyn AgentProtocol>);
        r.register_tool(make_tool("fail", "a1", &[])).await.unwrap();
    ...
    #[tokio::test]
    async fn test_registry_invoke_no_handler_returns_not_implemented() {
        let r = ToolRegistry::new();
        let agent = make_agent("a1");
        r.register_agent("a1", agent as Arc<dyn AgentProtocol>);
        // 注册工具但不注册 handler
        r.register_tool(make_tool("t", "a1", &[])).await.unwrap();
    ...
    #[tokio::test]
    async fn test_mock_agent_register_tool() {
        ...
        agent.register_tool(&def).await.unwrap();
        assert_eq!(agent.tool_count(), 1);
    }

    #[tokio::test]
    async fn test_mock_agent_execute_with_handler() {
        let agent = MockAgent::new("a1");
        agent.register_handler("echo", |args| Ok(args.clone()));
    ...
    #[tokio::test]
    async fn test_mock_agent_execute_without_handler() {
        let agent = MockAgent::new("a1");
        let req = AgentRequest {
            session_id: "s1".into(),
            method: "ghost".into(),
    ...
    #[tokio::test]
    async fn test_mock_agent_get_context_missing() {
        let agent = MockAgent::new("a1");
        let result = agent.get_context("ghost").await;
        assert!(result.is_err());
    }

    #[tokio::test]
    async fn test_mock_agent_get_context_present() {
        let agent = MockAgent::new("a1");
        let ctx = Context {
            session_id: "s1".into(),
            messages: vec![],
        ...
        let got = agent.get_context("s1").await.unwrap();
