...
const XDOOL = '/home/z/.local/pkg/root/usr/bin/xdotool';
...
(async () => {
  ...
  let winName = '';
  try { winName = execSync(`DISPLAY=:77 ${XDOOL} getactivewindowname 2>/dev/null`).toString().trim(); } catch {}
  let winFound = 0, winList = '';
  try {
    winList = execSync(`DISPLAY=:77 ${XDOOL} search --name "" 2>/dev/null || true`).toString().trim();
  ...
  try { execSync(`DISPLAY=:77 ${XDOOL} mousemove --sync 640 400 click 1`); } catch {}
  await page.waitForTimeout(800);
  try { execSync(`DISPLAY=:77 ${XDOOL} type --delay 45 "${NONCE} tauri-x-smoke"`); } catch (e) {}
  ...
  let wheelOk = true;
  try {
    for (let i = 0; i < 6; i++) { execSync(`DISPLAY=:77 ${XDOOL} click 4`); execSync(`DISPLAY=:77 ${XDOOL} click 5`); }
  } catch { wheelOk = false; }
  await page.waitForTimeout(800);
  ...
  try { execSync(`DISPLAY=:77 ${XDOOL} windowactivate --sync $(DISPLAY=:77 ${XDOOL} search --onlyvisible --name "." | head -1)`); } catch {}
  ...
  record('S5 X焦点切换→编辑器焦点可达', focus.pm || focus.tag === 'BODY', JSON.stringify(focus));
