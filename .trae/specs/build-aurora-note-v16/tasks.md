...
- [ ] Task 1.1: 实现事件溯源引擎
  - [ ] SubTask 1.1.1: 定义 Event 结构（event_id, block_id, op_type, payload, timestamp, user_id, device_id, signature）
  - [ ] SubTask 1.1.2: 实现聚合根（DocumentAggregate, BlockAggregate, WorkspaceAggregate）
  - [ ] SubTask 1.1.3: 实现快照策略（每 1000 个事件自动生成快照，启动加载最新快照 + 增量事件）
  - [ ] SubTask 1.1.4: 实现 SQLite WAL 模式事件存储，按 workspace 分库

- [ ] Task 1.2: 实现工作流引擎
  - [ ] SubTask 1.2.1: 实现状态机 DSL（基于 serde_json 描述状态节点与迁移条件）
  - [ ] SubTask 1.2.2: 实现触发器（时间触发、事件触发、API 触发）
  - [ ] SubTask 1.2.3: 实现异步任务执行器（tokio::sync::mpsc，支持重试与死信队列）

- [ ] Task 1.3: 实现权限引擎
  - [ ] SubTask 1.3.1: 实现 RBAC 五级角色（Owner/Admin/Editor/Commenter/Viewer）
  - [ ] SubTask 1.3.2: 实现 ABAC 属性条件扩展（如"仅工作日可编辑"、"仅特定 IP 可访问"）
  - [ ] SubTask 1.3.3: 实现权限传播（Workspace → Collection → Document → Block 四级层级继承）

- [ ] Task 1.4: 实现属性引擎
  - [ ] SubTask 1.4.1: 实现基础类型系统（Text/Number/Date/Checkbox/Select/MultiSelect/Relation/Formula）
  - [ ] SubTask 1.4.2: 实现基于 JSON Schema 的运行时类型校验
  - [ ] SubTask 1.4.3: 实现索引策略（热点属性自动建立 SQLite 索引，冷属性按需查询）

- [ ] Task 1.5: 实现查询引擎
  - [ ] SubTask 1.5.1: 实现统一 Query DSL（JSON 格式，支持过滤/排序/分页/聚合）
  - [ ] SubTask 1.5.2: 实现基于成本的查询优化器（自动选择 Tantivy/LanceDB/SQLite 执行路径）
  - [ ] SubTask 1.5.3: 实现 LRU 缓存层（TTL 基于数据变更频率动态调整）

- [ ] Task 1.6: 实现捕获引擎
  - [ ] SubTask 1.6.1: 实现管道架构（Source → Parser → Enricher → Normalizer → Storage）
  - [ ] SubTask 1.6.2: 实现来源插件接口（浏览器扩展、系统分享、邮件 IMAP、API Webhook）
  - [ ] SubTask 1.6.3: 实现去重策略（SimHash 内容哈希 + URL 指纹双重去重）

- [ ] Task 1.7: 实现 L1 基础设施 Trait 实现
  - [ ] SubTask 1.7.1: 实现 LoroCrdtEngine（基于 loro crate）
  - [ ] SubTask 1.7.2: 实现 IrohSyncTarget、WebSocketSyncTarget、LanSyncTarget
  - [ ] SubTask 1.7.3: 实现 LanceDbStore、SqliteVecStore
  - [ ] SubTask 1.7.4: 实现 SqliteStorage、SledStorage、S3Storage
  - [ ] SubTask 1.7.5: 实现 WasmtimeRuntime（WASM）、IframeRuntime（Web）
  - [ ] SubTask 1.7.6: 实现 McpAgentProtocol、NativeAgentProtocol

## Phase 2: L3 领域服务层 — P0 模块

- [ ] Task 2.1: 实现内容编辑系统
  - [ ] SubTask 2.1.1: 实现块级文档模型（Block 结构：id, block_type, content, properties, children；Document 结构）
  - [ ] SubTask 2.1.2: 实现块类型注册表（text/heading/code/image/table/divider/quote/list_item/todo_item，支持插件注册自定义类型）
  - [ ] SubTask 2.1.3: 实现富文本编辑核心（TipTap + ProseMirror Schema，pmToLoro/loroToPm 双向转换层）
  - [ ] SubTask 2.1.4: 实现 IME 合成期间暂停同步（compositionend 后批量提交）
  - [ ] SubTask 2.1.5: 实现 Markdown 支持（InputRule 实时转换 + 自定义序列化器导出 CommonMark）
  - [ ] SubTask 2.1.6: 实现协作编辑（基于 Loro 自动合并 + Awareness 协议协作光标）
  - [ ] SubTask 2.1.7: 实现版本历史（Loro Version Vector + 快照+增量存储 + 时间旅行 checkout）
  - [ ] SubTask 2.1.8: 实现评论批注（独立 Comment 模型 + 基于内容的锚定 + ProseMirror Decoration 渲染）

- [ ] Task 2.2: 实现知识网络系统
  - [ ] SubTask 2.2.1: 实现双链引用（WikiLink `[[标题]]` 和 MarkdownLink 解析，SQLite links 表索引）
  - [ ] SubTask 2.2.2: 实现链接索引异步更新（100ms debounce，监听 DocumentChanged 事件）
  - [ ] SubTask 2.2.3: 实现反向链接面板（反向查询 + 上下文预览 + 实时更新）
  - [ ] SubTask 2.2.4: 实现知识图谱可视化（D3.js 力导向图 + WebGL 加速 >1000 节点 + 聚类着色）
  - [ ] SubTask 2.2.5: 实现关系属性（语义标签：支持/反驳/引用/扩展 + Property Engine Relation 类型）
  - [ ] SubTask 2.2.6: 实现图谱探索（BFS 遍历 + LanceDB 相似度推荐 + 最短路径发现 + 探索历史记录）

- [ ] Task 2.3: 实现 GTD 效能系统 2.0
  - [ ] SubTask 2.3.1: 实现 GTD 工作流引擎（状态机：Inbox → Clarified → Organized → Scheduled → Doing → Done | Archived）
  - [ ] SubTask 2.3.2: 实现项目/任务无限层级嵌套（closure table 模式存储层级关系）
  - [ ] SubTask 2.3.3: 实现收件箱（快速创建 + 批量 Clarify + CaptureMatrix 内容默认入口）
  - [ ] SubTask 2.3.4: 实现重复任务（RRULE 库解析 iCalendar 标准 + 模板/实例分离模型）
  - [ ] SubTask 2.3.5: 实现提醒系统（tokio::time 定时器 + Desktop 系统通知 + Mobile APNs/FCM 推送）
  - [ ] SubTask 2.3.6: 实现习惯追踪（链式习惯模型 + streak 指标 + 热力图 + 习惯与任务联动）
  - [ ] SubTask 2.3.7: 实现时间线视图（日/周/月粒度 + 拖拽调整 due_date + 日历事件集成）
  - [ ] SubTask 2.3.8: 实现自动化规则（IFTTT 模式：When [Trigger] And [Condition] Then [Action]）

- [ ] Task 2.4: 实现 AI 智能系统
  - [ ] SubTask 2.4.1: 实现混合推理架构（AIProviderRouter：LocalFirst / CloudOnly / Auto 策略 + 运行时选择）
  - [ ] SubTask 2.4.2: 实现 AIProvider 实现（LlamaCppProvider 本地、OpenAIProvider 云端、OllamaProvider 本地 API）
  - [ ] SubTask 2.4.3: 实现智能续写（debounce 500ms + 流式 stream_complete + ghost text 显示 + Tab 接受/Esc 取消）
  - [ ] SubTask 2.4.4: 实现内容摘要（三级粒度：段落/文档/多文档 + Map-Reduce 策略 + SQLite 缓存关联版本号）
  ...
  - [ ] SubTask 2.4.8: 实现自动标签（TF-IDF + TextRank 关键词提取 + LLM 候选标签 + 用户确认）
  - [ ] SubTask 2.4.9: 实现任务分解（LLM 生成子任务列表 + 预览编辑 + 批量创建关联父任务）

## Phase 3: L3 领域服务层 — P1 模块

- [ ] Task 3.1: 实现安全加密系统
  - [ ] SubTask 3.1.1: 实现密钥层次结构（用户密码 → Argon2id 派生 Master Key → 每 Workspace 独立 DEK）
  - [ ] SubTask 3.1.2: 实现端到端加密（明文 → DEK AES-256-GCM 加密 → 存储/同步密文 + 零知识架构）
  - [ ] SubTask 3.1.3: 实现后量子加密（ML-KEM-768 与 X25519 双轨并行 + 混合密钥交换 + 算法迁移支持）
  - [ ] SubTask 3.1.4: 实现生物识别保护（Mobile FaceID/TouchID + Desktop TPM/Secure Enclave + 有效期配置）
  - [ ] SubTask 3.1.5: 实现密钥恢复（BIP39 助记词 + Shamir 秘密共享 3 选 2 + 设备授权 QR 码）

- [ ] Task 3.2: 实现同步服务系统
  - [ ] SubTask 3.2.1: 实现 P2P 同步（iroh + QUIC + NAT 穿透 + Loro Sync Protocol 版本向量交换）
  - [ ] SubTask 3.2.2: 实现云端同步（WebSocket 实时推送 + HTTPS 批量 + 密文中继 + 不可读存储）
  - [ ] SubTask 3.2.3: 实现局域网同步（mDNS 自动发现 + 局域网 IP 直连 QUIC + 优先于云端）
  - [ ] SubTask 3.2.4: 实现冲突解决（CRDT 自动合并 + 语义冲突 UI 手动选择 + 分支模式）
  - [ ] SubTask 3.2.5: 实现增量同步（CRDT ops 增量 + 媒体文件 rsync-like 块级增量 + zstd 压缩）
  - [ ] SubTask 3.2.6: 实现离线队列（SQLite sync_queue 表 + 优先级 + 幂等性 + 批量压缩）
  - [ ] SubTask 3.2.7: 实现多设备管理（Device ID Ed25519 + QR 码授权 + 远程撤销 + DEK 失效）

- [ ] Task 3.3: 实现插件管理系统
  - [ ] SubTask 3.3.1: 实现双模式架构（WASM 模式 + iframe 模式 + 插件生命周期管理）
  - [ ] SubTask 3.3.2: 实现 WASM 插件运行时（Wasmtime + WASI preview1 + 宿主函数 + Capability 清单控制）
  - [ ] SubTask 3.3.3: 实现 iframe 插件隔离（sandbox iframe + postMessage JSON-RPC 2.0 + CSS 变量透传）
  - [ ] SubTask 3.3.4: 实现插件权限控制（最小权限原则 + 运行时权限检查 + 动态升级）
  - [ ] SubTask 3.3.5: 实现插件市场（去中心化 + 官方/第三方/本地三种来源 + 代码签名 + 更新检查）
  - [ ] SubTask 3.3.6: 实现热更新（后台预加载 + 用户确认切换 + 失败回滚 + 灰度发布）

- [ ] Task 3.4: 实现导入导出系统
  - [ ] SubTask 3.4.1: 实现 Markdown 导入（pulldown-cmark 解析 → 中间 AST → ProseMirror 文档树 → Loro CRDT + YAML frontmatter）
  - [ ] SubTask 3.4.2: 实现 Notion 迁移（Notion API 导出 + 块类型映射 + 数据库映射 Collection + 富文本注释映射 mark 系统）
  - [ ] SubTask 3.4.3: 实现 PDF 导出（浏览器打印 headless Chrome + 自定义模板 + 批量导出）
  - [ ] SubTask 3.4.4: 实现批量导入（Zip 包 + manifest.json + 断点续传）

- [ ] Task 3.5: 实现素材库系统
  - [ ] SubTask 3.5.1: 实现内容寻址存储（SHA-256 哈希命名 + 天然去重 + SQLite assets 表元数据）
  - [ ] SubTask 3.5.2: 实现缩略图生成（image crate 异步 + ffmpeg 视频关键帧 + WebP 优先 + 按需延迟生成）
  - [ ] SubTask 3.5.3: 实现 EXIF 元数据解析（kamadak-exif + 拍摄时间/GPS/设备信息 + JSON 存储）
  - [ ] SubTask 3.5.4: 实现重复检测（精确 SHA-256 + 感知哈希 pHash / CNN Embedding 相似检测）

- [ ] Task 3.6: 实现系统设置
  - [ ] SubTask 3.6.1: 实现设置分层存储（系统级 → 用户级 → Workspace 级 + serde 强类型 + 版本迁移）
  - [ ] SubTask 3.6.2: 实现主题系统（CSS Variables 设计令牌 + Light/Dark/Sepia/HighContrast + Auto 跟随系统）
  - [ ] SubTask 3.6.3: 实现快捷键配置（全局快捷键 + 编辑器快捷键 + 冲突检测 + 平台适配 Ctrl/Cmd 映射）

- [ ] Task 3.7: 实现 TodayView 今日视图
  - [ ] SubTask 3.7.1: 实现聚合视图架构（QueryEngine 预计算查询 + Zustand 缓存 + EventBus 增量更新）
  - [ ] SubTask 3.7.2: 实现时间线视图（日/周/月粒度 + 虚拟列表 react-window + 拖拽回写 due_date）
  - [ ] SubTask 3.7.3: 实现专注模式（Pomodoro tokio::time 倒计时 + 25min/5min 循环 + 白噪音 + 专注统计）
  - [ ] SubTask 3.7.4: 实现每日回顾（自动生成当日报告 + 任务完成率/时间分配/习惯连续性 + 历史浏览）

- [ ] Task 3.8: 实现 OCR 服务
  - [ ] SubTask 3.8.1: 实现双引擎 OCR（PaddleOCR 主引擎中文 + Tesseract fallback 英文 + OCRProvider Trait 抽象）
  - [ ] SubTask 3.8.2: 实现图像预处理流水线（去噪 → 二值化 → 倾斜校正 → 版面分析）
  - [ ] SubTask 3.8.3: 实现表格识别（PP-Structure 版面分析 + 单元格独立 OCR + Markdown 表格重组）
  - [ ] SubTask 3.8.4: 实现公式识别（Texify 模型 + LaTeX 代码输出 + MathBlock 插入文档）
  - [ ] SubTask 3.8.5: 实现批量 OCR（tokio 线程池并行 + EventBus 进度推送 + 结果关联素材元数据）
