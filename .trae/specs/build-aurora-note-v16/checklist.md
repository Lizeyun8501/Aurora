...
## Phase 1: L2 核心引擎层
- [x] 事件溯源引擎：Event 结构包含完整字段（event_id, block_id, op_type, payload, timestamp, user_id, device_id, signature）
- [x] 事件溯源引擎：支持快照策略（每 1000 事件自动快照 + 启动加载快照+增量）
- [x] 事件溯源引擎：SQLite WAL 模式事件存储按 workspace 分库
- [x] 工作流引擎：支持 DSL 状态机定义和三种触发器（时间/事件/API）
- [x] 权限引擎：RBAC 五级角色 + ABAC 属性条件 + 四级层级权限传播
- [x] 属性引擎：8 种基础类型 + JSON Schema 校验 + 热点属性自动索引
- [x] 查询引擎：统一 Query DSL + 成本优化器自动选择执行路径 + LRU 缓存
- [x] 捕获引擎：管道架构完整 + SimHash + URL 指纹双重去重
- [x] L1 Trait 实现全部完成（LoroCrdtEngine, IrohSyncTarget, LanceDbStore, SqliteStorage, WasmtimeRuntime, McpAgentProtocol 等）

## Phase 2: P0 领域服务模块
- [x] 内容编辑：块级文档模型支持 9 种核心块类型 + 插件自定义类型注册
- [x] 内容编辑：TipTap/ProseMirror 与 Loro CRDT 双向绑定（pmToLoro/loroToPm）
- [x] 内容编辑：IME 合成期间暂停同步，compositionend 后批量提交
- [x] 内容编辑：Markdown 实时转换 + CommonMark/GFM/Aurora 扩展导出
- [x] 内容编辑：协作编辑基于 Loro 自动合并 + Awareness 协议协作光标
- [x] 内容编辑：版本历史支持时间旅行 checkout + 版本对比 diff + 分支合并
- [x] 内容编辑：评论批注使用基于内容的锚定 + ProseMirror Decoration 渲染
- [x] 知识网络：双链引用 WikiLink + MarkdownLink 解析 + SQLite links 表索引
- [x] 知识网络：链接索引 100ms debounce 异步更新
- [x] 知识网络：反向链接面板实时更新 + 上下文预览
- [x] 知识网络：知识图谱 D3.js 力导向图 + WebGL 加速 >1000 节点
- [x] 知识网络：关系属性支持语义标签（支持/反驳/引用/扩展）
- [x] 知识网络：图谱探索支持 BFS + 向量相似度推荐 + 最短路径发现
- [x] GTD：工作流状态机完整（Inbox → Clarified → Organized → Scheduled → Doing → Done | Archived）
- [x] GTD：项目/任务无限层级嵌套（closure table 模式）
- [x] GTD：重复任务 RRULE 解析 + 模板/实例分离
- [x] GTD：提醒系统 Desktop 系统通知 + Mobile APNs/FCM 推送
- [x] GTD：习惯追踪链式模型 + streak 指标 + 热力图
- [x] GTD：时间线视图日/周/月 + 拖拽调整 + 日历事件集成
- [x] GTD：自动化规则 IFTTT 模式（When/And/Then）
...
- [x] 安全加密：E2EE 明文 → AES-256-GCM → 密文存储/同步 + 零知识架构
- [x] 安全加密：后量子 ML-KEM-768 + X25519 双轨并行混合密钥交换
- [x] 安全加密：生物识别（FaceID/TouchID/Windows Hello + TPM/Secure Enclave）
- [x] 安全加密：密钥恢复 BIP39 助记词 + Shamir 3 选 2 + 设备 QR 码授权
- [x] 同步：P2P iroh + QUIC + NAT 穿透 + Loro Sync Protocol
- [x] 同步：云端 WebSocket 实时 + HTTPS 批量 + 密文中继
- [x] 同步：局域网 mDNS 发现 + IP 直连 + 优先于云端
- [x] 同步：冲突解决 CRDT 自动 + 语义冲突 UI + 分支模式
- [x] 同步：增量同步 ops 增量 + rsync-like 块级 + zstd 压缩
- [x] 同步：离线队列 SQLite + 优先级 + 幂等 + 批量压缩
- [x] 同步：多设备 Device ID + QR 授权 + 远程撤销 + DEK 失效
- [x] 插件：双模式 WASM + iframe + 完整生命周期
- [x] 插件：WASM Wasmtime + WASI + 宿主函数 + Capability 控制
- [x] 插件：iframe sandbox + postMessage JSON-RPC 2.0 + CSS 变量透传
- [x] 插件：最小权限 + 运行时检查 + 动态升级
- [x] 插件：市场去中心化 + 官方/第三方/本地 + 签名 + 热更新 + 灰度
- [x] 导入导出：Markdown pulldown-cmark 管道模式 + YAML frontmatter
- [x] 导入导出：Notion API 块类型映射 + 数据库映射 Collection
- [x] 导入导出：PDF 浏览器打印 + 自定义模板 + 批量
- [x] 导入导出：批量 Zip + manifest.json + 断点续传
- [x] 素材库：SHA-256 内容寻址 + 天然去重
- [x] 素材库：缩略图 image crate + ffmpeg + WebP + 按需生成
- [x] 素材库：EXIF kamadak-exif + GPS/时间/设备
- [x] 素材库：重复检测 pHash / CNN Embedding
- [x] 系统设置：分层存储 + serde 强类型 + 版本迁移
- [x] 系统设置：主题 CSS Variables + Light/Dark/Sepia/HighContrast/Auto
- [x] 系统设置：快捷键全局+编辑器 + 冲突检测 + 平台适配
- [x] TodayView：聚合视图 QueryEngine 预计算 + Zustand + EventBus 增量
